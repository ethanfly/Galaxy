//! Google Antigravity CLI: `~/.gemini/antigravity/conversations/` (read-only).
...
impl AntigravityAdapter {
    fn bases() -> Vec<PathBuf> {
        ...
        if let Some(h) = home_dir() {
            out.push(h.join(".gemini").join("antigravity").join("conversations"));
            out.push(h.join(".gemini").join("antigravity"));
            out.push(h.join(".antigravity").join("conversations"));
            out.push(h.join(".antigravity"));
        }
        out
    }

    fn collect(cancel: &CancelToken) -> Vec<PathBuf> {
...
impl AgentAdapter for AntigravityAdapter {
    ...
    fn availability(&self) -> AgentAvailability {
        ...
        AgentAvailability {
            ...
            reason: if ok {
                ...
                "未找到 Antigravity 会话目录（~/.gemini/antigravity）".into()
            },
        }
    }
    ...
    fn scan(
        ...
        let leaf = project_path
            ...
            .to_lowercase();
        let mut out = Vec::new();
        for path in Self::collect(cancel) {
            ...
            let summary = first_nonempty(&lines, "/title")
                ...
                .unwrap_or_else(|| format!("Antigravity · {id}"));
            out.push(AgentConversation {
                agent_kind: AgentKind::Antigravity,
                external_id: id.clone(),
                project_path: project_path.to_string(),
                summary: summary.chars().take(120).collect(),
                last_message_at: None,
                status: AgentStatus::Idle,
                resume_command: format!("antigravity --resume {id}"),
    ...
    fn resume_command(&self, conv: &AgentConversation) -> String {
        format!("antigravity --resume {}", conv.external_id)
    }
}

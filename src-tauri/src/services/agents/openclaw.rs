//! OpenClaw: `~/.openclaw/agents/` and legacy `~/.clawdbot/agents/` (read-only).
...
impl OpenClawAdapter {
    fn bases() -> Vec<PathBuf> {
        ...
        if let Some(h) = home_dir() {
            out.push(h.join(".openclaw").join("agents"));
            out.push(h.join(".openclaw").join("sessions"));
            out.push(h.join(".openclaw"));
            out.push(h.join(".clawdbot").join("agents"));
            out.push(h.join(".clawdbot"));
        }
        out
    }
...
impl AgentAdapter for OpenClawAdapter {
    ...
    fn scan(
        ...
        for path in files {
            ...
            let summary = first_nonempty(&lines, "/title")
                ...
                .unwrap_or_else(|| format!("OpenClaw · {id}"));
            out.push(AgentConversation {
                agent_kind: AgentKind::OpenClaw,
                external_id: id.clone(),
                project_path: project_path.to_string(),
                summary: summary.chars().take(120).collect(),
                last_message_at: None,
                status: AgentStatus::Idle,
                resume_command: format!("openclaw --session {id}"),
    ...
    fn resume_command(&self, conv: &AgentConversation) -> String {
        format!("openclaw --session {}", conv.external_id)
    }
}

...
impl AgentAdapter for OpenCodeAdapter {
    ...
    fn availability(&self) -> AgentAvailability {
        ...
        match Self::open_readonly(&path) {
            Some(conn) => match Self::columns(&conn, "session") {
                Some(_) => AgentAvailability { kind: Some(self.kind()), available: true, reason: String::new() },
    ...
    fn scan(&self, project_path: &str, _since_ms: u64, _cancel: &CancelToken) -> Vec<AgentConversation> {
        ...
        let Some(conn) = Self::open_readonly(&path) else { return Vec::new() };
        let Some(cols) = Self::columns(&conn, "session") else { return Vec::new() };
        ...
        let time_col = Self::pick(&cols, &["time_updated", "updated_at", "time_created", "created_at"]);
        let Some(dir_col) = dir_col else { return Vec::new() };
        ...
        for row in rows.flatten() {
            ...
            out.push(AgentConversation {
                ...
                summary: title.filter(|t| !t.is_empty()).unwrap_or_else(|| "OpenCode 会话".into()),
                last_message_at: at,
    ...
    fn read_messages(&self, conv: &AgentConversation, limit: usize) -> Vec<AgentMessage> {
        ...
        let Some(conn) = Self::open_readonly(std::path::Path::new(db)) else { return Vec::new() };
        let Some(cols) = Self::columns(&conn, "message") else { return Vec::new() };
        let session_col = Self::pick(&cols, &["session_id", "sessionId", "session"]).unwrap_or_else(|| "session_id".into());
